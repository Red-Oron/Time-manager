...
def barh(steps):
    ...
    for i, activity in enumerate(steps):
        pivot = steps[i + 1][1] if i < len(steps) - 1 else datetime.now().timestamp()
    ...
    START_DAY = datetime.fromtimestamp(steps[0][1]).weekday()
    ...
    for i in range(len(steps)):
        ...
        if i != len(steps) - 1:
            next_activity_time = datetime.strptime(steps[i + 1][2], "%Y-%m-%d %H:%M:%S")
        else:
    ...
    def format_coord(x, y):
        ...
        if not steps[0][1] <= timestamp <= datetime.now().timestamp(): week = ''

        for i in steps:
            if i[1] >= timestamp or not steps[0][1] <= timestamp <= datetime.now().timestamp(): break
            note = i[-1]
