...
force_exit = False
save_activities = set([i[0] for i in save.steps])
...
for i, activity in enumerate(save.steps):
    pivot = save.steps[i + 1][1] if i < len(save.steps) - 1 else datetime.now().timestamp()
...
START_DAY = datetime.fromtimestamp(save.steps[0][1]).weekday()
EXPERIMENT_START_TIME = save.steps[0][1]
...
fig, axs = plt.subplot_mosaic(
    ...
    gridspec_kw={
        "height_ratios": [(ALL_EXPERIMENT_TIME // (24 * h) + 2 if ALL_EXPERIMENT_TIME >= 48 * h else 2) if FULL else 14,
                          1]}
)
...
for activity in activities_times:
    ...
    print(
        f"{activity}: {round(AVERAGE_DAY[activity] / h, 2)}ч ({round(AVERAGE_DAY[activity] / ALL_EXPERIMENT_TIME * 100, 1)}%)")
...
for i in range(len(save.steps)):
    activity = save.steps[i]

    if i != len(save.steps) - 1:
        next_activity_time = datetime.strptime(save.steps[i + 1][2], "%Y-%m-%d %H:%M:%S")
    else:
        next_activity_time = datetime.utcfromtimestamp(datetime.now().timestamp())

    this_activity_time = datetime.strptime(activity[2], "%Y-%m-%d %H:%M:%S")

    activity_max_time = datetime.strptime(activity[2][:10] + " 23:59:59", "%Y-%m-%d %H:%M:%S")
    activity_min_time = datetime.strptime(activity[2][:10] + " 00:00:00", "%Y-%m-%d %H:%M:%S")
...
def format_coord(x, y):
    ...
    if not save.steps[0][1] <= timestamp <= datetime.now().timestamp(): week = ''

    for i in save.steps:
        if i[1] >= timestamp or not save.steps[0][1] <= timestamp <= datetime.now().timestamp(): break
        note = i[-1]
